...
| recorded tasks | amber | 0.2.6, three 5-run passes alternating with 0.2.5: faster on every task in all but one pass, Flights 14/15 (one `unverified_done`); oliveyoung 2.71x under the 3x bar in its one unwalled pass, jev-ra on the TypeSafe direct route on 2026-09-23 against browser-use's OpenRouter runs of 2026-09-18, which were not repeated (docs/BENCHMARKS.md, 0.2.6); 0.2.7 adds the human-check handoff and overlaps 0.2.6 on every task, 10/10 each across two alternating passes (docs/BENCHMARKS.md, 0.2.7) |
