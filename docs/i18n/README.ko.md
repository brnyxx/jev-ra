...
사람인지 확인하려는 사이트(CAPTCHA 위젯, Cloudflare의 "Just a moment...", 길게 누르기 확인)는 사람에게
넘긴다. 에러 페이지처럼 먼저 물러났다가 페이지를 한 번 더 요청하고, 그래도 확인이 남아 있으면 Chrome 창을
앞으로 가져오고 사이트 이름을 담은 데스크톱 알림을 띄운 뒤(`JEV_RA_NOTIFY=0`이면 끈다) 페이지가 더는
확인이 아니게 될 때까지 최대 120 s(`JEV_RA_HUMAN_WAIT_S`) 기다린다. 그다음 같은 실행이 같은 목표, 기록,
값과 남은 스텝으로 이어진다. 기다린 시간은 `Result.human_wait_ms`에 따로 담기고 스텝 예산도 시간 예산도
쓰지 않는다. 제시간에 아무도 풀지 않으면 실행은 `needs_human`으로 멈추고, `detail`에 사이트와 확인 종류,
그리고 실행 id인 `resume`이 담긴다. 사람이 확인을 푼 뒤 `browser_run(resume=...)`이나
`jev-ra run --resume ID`를 부르면 처음부터가 아니라 그 페이지에서 이어 간다. headless 브라우저나 원격 CDP
주소의 브라우저는 확인을 보여 줄 사람이 없으므로 실행은 곧바로 `needs_human`으로 멈추고,
`detail.next_step`에 사람이 볼 수 있는 곳에서 다시 돌리는 방법이 적힌다. "Access denied", 확인 없는 403,
지역 차단처럼 아예 거절하는 사이트는 여전히 `blocked_by_site`이고 `detail.kind`는 `refusal`이다. jev-ra는
확인을 대신 풀지 않고, 자동화임을 숨기지 않으며, 다른 프로필의 쿠키를 빌려 오지 않는다.

실행이 스스로 시작하는 이동(처음 여는 것과 한 번 쓸 수 있는 재요청)은 호스트마다 간격을 둔다. 최소 1 s
(`JEV_RA_PACE_S`) 떨어뜨리고, 429로 답하거나 확인을 띄운 호스트는 연달아 그럴 때마다 다음 이동을 두 배씩
더 기다리게 한다. 실행은 주소를 한 번만 열기 때문에 보통의 실행은 붙잡히지 않고, 실행이 도는 머신 자신에는
간격을 두지 않는다.
