...
Python을 따로 갖추기 싫다면 `npx -y jev-ra install claude`가 0.1.0 릴리스 이후 npm 런처로 같은 일을
한다([Releases](https://github.com/brnyxx/jev-ra/releases)).
...
모두 페이지 텍스트와 순위가 매겨진 후보를 담은 깨끗한 에스컬레이션이다. 크래시도, 조용히 잘못된
동작을 하는 것도 아니다.
## FAQ
