...
人間であることの確認を求めるサイト(CAPTCHA ウィジェット、Cloudflare の "Just a moment..."、長押しの確認)は、
人に引き渡す。エラーページと同じく一度間を置いてページをもう一度要求し、それでも確認が残っていれば
Chrome のウィンドウを前面に出し、サイト名を示すデスクトップ通知を出して(`JEV_RA_NOTIFY=0` で止まる)、
ページが確認でなくなるまで最大 120 s(`JEV_RA_HUMAN_WAIT_S`)待つ。その後は同じ実行が、同じゴール、履歴、
値と残りのステップで続く。待った時間は `Result.human_wait_ms` に別に記録され、ステップ予算も時間予算も
使わない。時間内に誰も解かなければ実行は `needs_human` で止まり、`detail` にはサイト、確認の種類、
実行 id である `resume` が入る。人が確認を解いたあとで `browser_run(resume=...)` か
`jev-ra run --resume ID` を呼べば、最初からではなくそのページから続く。headless のブラウザやリモート CDP
のブラウザには確認を見せる相手がいないので、実行はすぐに `needs_human` で止まり、`detail.next_step` に
人が見られる場所で実行し直す方法が書かれる。"Access denied"、確認のない 403、地域制限のようにはっきり
拒むサイトは従来どおり `blocked_by_site` で、`detail.kind` は `refusal` になる。jev-ra は確認を代わりに
解かず、自動化であることを隠さず、他のプロファイルの Cookie を借りない。

実行が自分で始める遷移(最初のオープンと、使うかもしれない一度の再読み込み)はホストごとに間隔を空ける。
最低 1 s(`JEV_RA_PACE_S`)離し、429 を返したか確認を出したホストには、続くたびに次の遷移を倍ずつ長く
待たせる。実行はアドレスを一度しか開かないので普通の実行が待たされることはなく、実行しているマシン自身は
対象外だ。
