...
各実行は最終ページに対して検証し、25 回中 25 回が成功、テキストモデルの呼び出しは 0 回だった。
...
われわれの中央値を browser-use のタスク別最速記録(15,759 ms、49,914 ms、17,647 ms)と比べると
5.8×、5.6×、4.6×で、見出しの 3-5× はそれより低い値だ。
...
サイト)にわたる実サイトの 83 タスクで、各タスクには実行が残したページを検査する仕様がある。0.2.4 で各 3 回:
**213 / 249 = 85.5 %**。両ツールに同じく与えた 40 タスクで、browser-use 0.13.10 `flash_mode` は
**29 / 40 = 72 %**、中央値 **19.4 s**。jev-ra は **102 / 120 = 85 %**、中央値 **3.1 s**。
