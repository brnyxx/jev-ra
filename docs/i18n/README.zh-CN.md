...
要求证明是真人的站点(CAPTCHA 组件、Cloudflare 的 "Just a moment..."、长按验证)会交给人来处理。和错误页
一样，先退避并重新请求一次页面；如果验证仍在，就把 Chrome 窗口调到前台，弹出写明站点名的桌面通知
(`JEV_RA_NOTIFY=0` 关闭)，并最多等待 120 s(`JEV_RA_HUMAN_WAIT_S`)，直到页面不再是验证。之后同一次运行
继续：同样的目标、历史和值，以及剩下的步数。等待时间单独记在 `Result.human_wait_ms` 中，不占用步数预算，
也不占用时间预算。若在时限内无人完成，运行以 `needs_human` 停止，`detail` 写明站点、验证类型以及运行 id
`resume`。人完成验证后调用 `browser_run(resume=...)` 或 `jev-ra run --resume ID`，运行会从那一页继续，
而不是从头开始。headless 浏览器或远程 CDP 地址上的浏览器没有人能看到验证，因此运行会立即以
`needs_human` 停止，`detail.next_step` 说明如何在有人能看到的地方重新运行。"Access denied"、没有验证的
403、地区封锁这类直接拒绝的站点仍是 `blocked_by_site`，`detail.kind` 为 `refusal`。jev-ra 从不代为完成
验证，从不隐藏自己是自动化，也从不借用其他配置文件的 Cookie。

运行自己发起的导航(首次打开，以及可能用到的那一次重新加载)按主机设定间隔：至少相隔 1 s(`JEV_RA_PACE_S`)；
对返回 429 或弹出验证的主机，每连续出现一次，下一次导航就多等一倍。运行只打开一次地址，所以普通运行不会
被拖住；运行所在的本机不受限制。
